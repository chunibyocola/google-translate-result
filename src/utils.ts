import { CONNECTION_TIMED_OUT, BAD_REQUEST } from './errorCodes';
...
export const getQueryString = (params: any) => {
    ...
    Object.keys(params).map((v, i) => {
        ...
        Array.isArray(params[v]) ?
            search += params[v].reduce((t: string, c: string, i: number) => (`${i ? t + '&' : ''}${v}=${c}`), '') :
            search += `${v}=${params[v]}`;
...
export const fetchData = async (url: string, init = {}) => {
    ...
    if (!res.ok) { throw getError(`${BAD_REQUEST}`); }
...
export const isLangCodeSupported = (langCode: string) => {
    return langCode in langCodes;
};